...
export function initToolkitApp({ tools = [], stacks = [], brand = BRAND_CONFIG } = {}) {
  ...
  // ── Playbook card view switching ──
  const playbookCards = document.querySelectorAll('.playbook-card');
  const playbookState = {};

  // Pre-compute resolved tools and alternatives for each stack
  stacks.forEach((stack, idx) => {
    const resolved = stack.tools.map((name) => allTools.find((t) => t.name === name)).filter(Boolean);
    const freeTools = resolved.map((tool) => {
      const alt = findBestAlternative(allTools, tool.name, 'free');
      return alt || tool;
    });
    const paidTools = resolved.map((tool) => {
      const alt = findBestAlternative(allTools, tool.name, 'paid');
      return alt || tool;
    });
    playbookState[idx] = {
      advised: resolved,
      advisedNames: stack.tools,
      free: freeTools,
      premium: paidTools,
      activeView: 'advised',
    };
  ...
  function renderPlaybookTools(stackIndex) {
    const container = document.querySelector(`[data-stack-tools="${stackIndex}"]`);
    if (!container) return;
    const info = playbookState[stackIndex];
    if (!info) return;

    const viewTools = info[info.activeView] || info.advised;
    container.innerHTML = '';
    ...
    viewTools.forEach((tool) => {
      ...
      const colorMap = {
        advised: 'bg-gray-800/80 text-gray-300',
        free: 'bg-emerald-900/30 text-emerald-300 border border-emerald-800/40',
        premium: 'bg-purple-900/30 text-purple-300 border border-purple-800/40',
      };
      span.className = `px-2 py-0.5 rounded text-[11px] font-medium ${colorMap[info.activeView] || colorMap.advised}`;
      span.textContent = `${tool.name} ${pricing.icon}`;
      container.appendChild(span);
    });
  }

  document.querySelectorAll('.playbook-view-btn').forEach((btn) => {
    btn.addEventListener('click', (event) => {
      const target = event.currentTarget;
      const stackIdx = Number(target.dataset.stack);
      const view = target.dataset.view;
      if (!playbookState[stackIdx]) return;

      playbookState[stackIdx].activeView = view;

      // Update button states for this card
      const card = target.closest('.playbook-card');
      if (card) {
        card.querySelectorAll('.playbook-view-btn').forEach((b) => {
          b.classList.remove('active-view');
          b.className = b.className.replace(/text-blue-400|text-emerald-400|text-purple-400/g, 'text-gray-500');
          b.className = b.className.replace(/bg-blue-900\/20|bg-emerald-900\/20|bg-purple-900\/20/g, '');
          b.className = b.className.replace(/border-blue-500|border-emerald-500|border-purple-500/g, 'border-transparent');
        });
        const colorMap = {
          advised: { text: 'text-blue-400', bg: 'bg-blue-900/20', border: 'border-blue-500' },
          free: { text: 'text-emerald-400', bg: 'bg-emerald-900/20', border: 'border-emerald-500' },
          premium: { text: 'text-purple-400', bg: 'bg-purple-900/20', border: 'border-purple-500' },
        };
        const colors = colorMap[view] || colorMap.advised;
        target.classList.add('active-view');
        target.classList.remove('text-gray-500', 'border-transparent');
        target.classList.add(colors.text, colors.bg, colors.border);
      }

      renderPlaybookTools(stackIdx);
    });
  });
