...
export function initToolkitApp({ tools = [], stacks = [], brand = BRAND_CONFIG } = {}) {
  ...
  document.querySelectorAll('.playbook-view-btn').forEach((btn) => {
    btn.addEventListener('click', (event) => {
      ...
      if (!Number.isFinite(stackIdx) || !playbookState[stackIdx]) return;
      ...
      const allActiveClasses = [
        ...
        'bg-blue-900/20', 'bg-emerald-900/20', 'bg-purple-900/20',
      ...
      if (card) {
        ...
        target.classList.add('active-view', colors.text, colors.bg, colors.border);
      }
